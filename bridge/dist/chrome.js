...
import { CDP_BROWSER_URL, CDP_PORT, CHROME_BINARY, CHROME_USER_DATA_DIR, EXTENSION_PATH, } from "./config.js";
import { probeCdp } from "./mcp.js";
/** 叫出 macOS 原生「選擇資料夾」對話框，回傳絕對路徑。 */
export function pickFolder(prompt = "選擇 automatic-testing 專案資料夾") {
    return new Promise((resolve) => {
        ...
        p.on("close", (code) => {
            if (code === 0 && out.trim())
                resolve({ path: out.trim().replace(/\/+$/, "") });
            else if (/User canceled|cancel/i.test(err))
                resolve({ canceled: true });
            else
                resolve({ error: err.trim() || `osascript exit ${code}` });
        });
    });
}
/** 輪詢 CDP 直到可連或逾時。 */
 ...
 */
export async function launchChrome(url = "https://example.com") {
    // 已在跑 → 直接回
    const existing = await probeCdp(CDP_BROWSER_URL);
    if (existing.ok)
        return { ok: true, alreadyRunning: true, version: existing.version };
    ...
    if (!ready)
        return { ok: false, error: "Chrome 已啟動但 CDP 在逾時內未就緒" };
    const p = await probeCdp(CDP_BROWSER_URL);
