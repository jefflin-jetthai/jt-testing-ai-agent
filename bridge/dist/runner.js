...
import { ARTIFACTS_DIR, AT_REPO_PATH, CDP_BROWSER_URL, CLAUDE_MODEL, DEFAULT_AGENT, } from "./config.js";
import { getAgent } from "./agents/index.js";
import { CHROME_DEVTOOLS_TOOLS, JT_BROWSER_TOOLS, probeCdp, writeBrowserMcpConfig, writeMcpConfig, } from "./mcp.js";
...
export async function startRun(payload, emit) {
    ...
    if (mode === "attach") {
        ...
        if (!probe.ok) {
            throw new Error(`無法連到目標 Chrome 的 CDP（${CDP_BROWSER_URL}）。\n` +
        ...
        for (const tc of payload.cases) {
            ...
            try {
                const pageWs = canRecord
                    ...
                    : null;
                if (pageWs) {
                    recorder = new ScreencastRecorder(pageWs, join(runDir, `${tc.tcId}.gif`), join(runDir, `.tmp-${tc.tcId}`));
            ...
            if (recorder) {
                try {
                    ...
                    if (gifPath)
                        log({ tcId: tc.tcId, kind: "system", text: `🎞 已產出錄影：${gifPath}` });
                }
                catch (e) {
