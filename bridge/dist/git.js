...
export async function currentBranch(cwd = AT_REPO_PATH) {
    const r = await git(["rev-parse", "--abbrev-ref", "HEAD"], cwd);
    return r.stdout.trim();
}
/** porcelain 狀態（限定 tests/ 與 specs/ 路徑，避免動到無關檔案）。 */
export async function changedTestFiles(cwd = AT_REPO_PATH) {
    const r = await git(["status", "--porcelain", "--untracked-files=all", "--", "tests/", "specs/"], cwd);
    return r.stdout
        .split("\n")
        .map((l) => l.slice(3).trim())
        .filter(Boolean);
}
export async function diff(files, cwd = AT_REPO_PATH) {
...
export async function createCommit(args) {
    ...
    if (!args.files.length)
        return { ok: false, branch: await currentBranch(cwd), error: "沒有要提交的檔案" };
    if (args.branch) {
        ...
        if (co.code !== 0) {
            ...
            if (co2.code !== 0)
                return { ok: false, branch: args.branch, error: co.stderr || co2.stderr };
        }
    }
    const add = await git(["add", "--", ...args.files], cwd);
    if (add.code !== 0)
        return { ok: false, branch: await currentBranch(cwd), error: add.stderr };
    const commit = await git(["commit", "-m", args.message], cwd);
    const branch = await currentBranch(cwd);
    if (commit.code !== 0)
        return { ok: false, branch, error: commit.stderr || commit.stdout };
    const hash = (await git(["rev-parse", "HEAD"], cwd)).stdout.trim();
    return { ok: true, branch, hash };
}
/** 明確 push（使用者於 UI 觸發）。 */
export async function push(branch, cwd = AT_REPO_PATH) {
